export default async function handler(req, res) {
  ...
  const systemPrompt = `你是一个专业的短视频文案分析师。只返回JSON，不要其他内容。
...
structure选项：身份介绍/人群圈定、痛点切入/痛点解决、产品引入、成分介绍/认知科普、产品工艺/产品理念、信任背书、使用场景/使用方法、效果承诺/效果佐证、体验展示/种草体验、高端关联/产品情怀、促销机制/产品价格`;
  try {
    const resp = await fetch('https://api.deepseek.com/chat/completions', {
      ...
      body: JSON.stringify({ model: 'deepseek-chat', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: `文案内容：\n${content}` }], max_tokens: 1024 }),
    });
    const data = await resp.json();
    if (!resp.ok) return res.status(500).json({ error: data?.error?.message || 'AI分析失败' });
